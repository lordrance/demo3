...
const PanelPage = () => {
	...
	const changeDirection = (e: any) => {
		if (e === 1) {
			if (page.pageHeight > page.pageWidth) {
				dispatch(
					changeTempPageWH({
						pageWidth: page.pageHeight,
						pageHeight: page.pageWidth,
					}),
				);
	...
	const changePaper = (s: string) => {
		const wh = s.split(",");
		const e: number[] = [];
		e[0] = Number.parseInt(wh[0]);
		e[1] = Number.parseInt(wh[1]);
		...
		<>
			...
			<div className=" space-y-[20px] px-5 p-3">
				<div className="flex gap-1 items-center">
					<label className=" shrink-0">模板名称</label>
					<Input
						placeholder="请输入模板名称"
						value={page.name}
				...
				<div>
					...
					<InputNumber
						...
						onChange={(e: any) => {
							dispatch(
								changeTempPageWH({ pageWidth: e, pageHeight: page.pageHeight }),
							);
				...
				<div>
					...
					<InputNumber
						...
						onChange={(e: any) => {
							dispatch(
								changeTempPageWH({ pageWidth: page.pageWidth, pageHeight: e }),
							);
