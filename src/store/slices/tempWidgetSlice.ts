...
import { findClosestIndices, insertIntoArray, uniqueArray } from "@/utils/move";
	...
	reducers: {
		...
		createWidgets(state, { payload }: { payload: Widget[] }) {
			...
			payload.forEach((x) => {
				...
				posY[c] = x.top + bw;
				c++;
				posX[c] = x.left + x.width;
				posY[c] = x.top + x.height;
				c++;
			});
			posX.sort();
			posY.sort();
			posX = uniqueArray(posX);
			posY = uniqueArray(posY);
			near = [];
		},
		//改变当前活跃组件
		...
		//删除当前活跃组件
		deleteWidget(state) {
			if (state.activeIndex !== -1) state.widgets.splice(state.activeIndex, 1);
			state.activeIndex = -1;
		},
		//设置组件样式
